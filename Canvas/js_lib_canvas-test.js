...
//////////////////////////////////////////////////
// Course list modifications					//
//////////////////////////////////////////////////

// Identify if we're on the all courses page.
if( document.URL.search(/upskilled(\.test|\.beta)?\.instructure\.com\/courses$/i) > -1 ) {
	// Rename the titles
	var title = document.querySelector('.header-bar h1');
	if(title) {
		title.innerHTML = 'My Courses';
	}
	var futureTitle = document.querySelector('.content--hasMarginTop h2');
	if(futureTitle) {
		futureTitle.innerHTML = 'Upcoming Courses';
	}
	// The table is filled after page load, so keep trying until it's there
	var courseListTimer = setInterval( modifyCourseList, 50 );
}

function modifyCourseList() {
	// Modify the main table
	var courseTable = document.querySelector('#my_courses_table');
	if( courseTable ) {
		// Rename 'Published' column to 'Active'
		var pubColumn = courseTable.querySelector('th.course-list-published-column');
		pubColumn.innerHTML = 'Active';

		// Remove columns
		var termColumn = courseTable.querySelectorAll('td.course-list-term-column, th.course-list-term-column');
		termColumn.forEach(function(cell) {
			cell.remove();
		});
		var nicknameColumn = courseTable.querySelectorAll('td.course-list-nickname-column, th.course-list-nickname-column');
		nicknameColumn.forEach(function(cell) {
			cell.remove();
		});
		var enrolledAsColumn = courseTable.querySelectorAll('td.course-list-enrolled-as-column, th.course-list-enrolled-as-column');
		enrolledAsColumn.forEach(function(cell) {
			cell.remove();
		});

		// Do the same for the future enrollments table
		var futureTable = document.querySelector('#future_enrollments_table');
		// Rename 'Published' column to 'Active'
		pubColumn = futureTable.querySelector('th.course-list-published-column');
		pubColumn.innerHTML = 'Active';

		// Remove columns
		termColumn = futureTable.querySelectorAll('td.course-list-term-column, th.course-list-term-column');
		termColumn.forEach(function(cell) {
			cell.remove();
		});
		nicknameColumn = futureTable.querySelectorAll('td.course-list-nickname-column, th.course-list-nickname-column');
		nicknameColumn.forEach(function(cell) {
			cell.remove();
		});
		enrolledAsColumn = futureTable.querySelectorAll('td.course-list-enrolled-as-column, th.course-list-enrolled-as-column');
		enrolledAsColumn.forEach(function(cell) {
			cell.remove();
		});

		// Stop trying once we've been able to modify the table
		clearInterval(courseListTimer);
	}
}
