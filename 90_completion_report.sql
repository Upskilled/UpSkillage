...
from
	(select
		trainer_name, course, student_name, from_unixtime(last_grade_timestamp) as last_grade,
		case when (completed_assessment_items >= round(total_assessment_items * 0.9)) then 'yes' else 'no' end as completed_90_percent,
	...
	from
		(select
			...
			sum(case when (gg.finalgrade/gg.rawgrademax >= 0.70) then 1 else 0 end) as completed_assessment_items,
		...
		from
			prefix_user u
			inner join prefix_user_enrolments ue on u.id = ue.userid
			inner join prefix_enrol e on e.id = ue.enrolid
			inner join prefix_course c on c.id = e.courseid and c.shortname not like '%LLN%' and c.startdate >= unix_timestamp('2011-01-01')
			inner join prefix_grade_items gi on c.id = gi.courseid
			left outer join prefix_grade_grades gg on gi.id = gg.itemid and gg.userid = u.id
			%%FILTER_ENDTIME:gg.timemodified:<%%
			left outer join prefix_course_completions cc on cc.userid = u.id and cc.course = c.id
		where
			gi.itemtype = 'mod'
...
where cmp90.completed_90_percent = 'yes'